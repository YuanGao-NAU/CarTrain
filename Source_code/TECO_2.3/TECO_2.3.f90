...
!    real paraest(19,40000)
    ...
    integer seq,Pselect
    character(len=150) parafile,daparfile,outdir,uniparfile
    character(len=150) paraestfile
    integer,parameter :: partotal=58
...
!    real coefhistory(ncov,npara)
    ...
    character(len=150) Targu,CO2argu, repnargu
...
!   for initialize
    ...
    integer yrs_eq,rep,yrlim,dylim, repn
...
!    logical, parameter :: is_grass = .True.   ! grassland
    ...
    character(len=150) :: namelistfile

    namelist /control_pm/ is_grass,first_year,use_plinit,do_soilphy,&
    ...
    open(517,file=trim(namelistfile))
    read(517,nml=control_pm)

    ! get input file name
    call getarg(2, parafile)
    call getarg(3, DAparfile)
    call getarg(4, climatefile1)
    call getarg(5, outdir)
    call getarg(6, obsfile_cflux)
    call getarg(7, obsfile_cpool)
    call getarg(8, obsfile_ch4flux)
    call getarg(9,Targu)
    call getarg(10,CO2argu)
    call getarg(11, repnargu)
    read(repnargu,'(I3)') repn
...
!    shcap_snow=1000000.  ! tuneice worker better  !!commented import from input file 0811 ms
!    condu_snow=0.1                                !!commented import from input file 0811 ms    
!    condu_b = 0.08  ! yuanyuan soil thermal version  this par is not sensitive to CWE!!commented import from input file 0811 ms
!    albedo_snow=0.7 !!commented import from input file 0811 ms    
!    fa = 1   !!commented import from input file 0811 ms
!    fsub=0.1 !!commented import from input file 0811 ms    
!    rho_snow=80.        !tuneice  !!commented import from input file 0811 ms
!    decay_m=2.2192      !aging factor on snow melting  !!commented import from input file 0811 ms    
!    depth_ex=0.05      !!commented import from input file 0811 ms
! ***********   end of soil thermal paras initial values 
    
!   Read parameters from file
!    call getarg(1,parafile)
!    parafile='input/SPRUCE_pars.txt'
!    parafile='input/CiPEHR_pars.txt'    ! test grassland
    ...
    &   shcap_snow,condu_snow,condu_b,albedo_snow,fa,fsub,rho_snow,decay_m,depth_ex)  ! added for cwt DA
...
!        write (*,*) "forcing data", forcing_data1
...
!            & ice5,ice6,ice7,ice8,ice9,ice10,zwt'
            write(63,*)'day,wcl1,wcl2,wcl3,wcl4,wcl5,wcl6,wcl7,wcl8,wcl9,wcl10,liq_water1,liq_water2,liq_water3,&
            & liq_water4,liq_water5,liq_water6,liq_water7,liq_water8,liq_water9,liq_water10,wsc1,wsc2,wsc3,wsc4,&
            & wsc5,wsc6,wsc7,wsc8,wsc9,wsc10,zwt'
...
!    if (MCMC.eq.2) GOTO 150
    ...
    yrs_eq=yr_length*4  ! spin up length in simulation mode 
    
!    yrs_eq=18!+5+5 !18+8+5+5
    ...
    call TECO_simu(MCMC,Simu_dailyflux,Simu_soilwater,      &
     ...
     &    first_year,yr_length1,nyrmax,is_grass,daily)
...
!    DAparfile='input/SPRUCE_da_pars_c.txt'
    ...
    write(uniparfile,"(A120,A11)") trim(outdir),"/unipar.txt"
    uniparfile = trim(uniparfile)
    uniparfile = adjustl(uniparfile)
    print *, "uniparfile=", uniparfile
    print *, "paraestfile=", paraestfile
...
!    Pselect = 20
    ...
    enddo  
    write (*,*) 'npara=',npara
    if (npara .gt. 8.) then         ! assign parameter values in forecasting mode
        SLA = parval(12)
        GLmax = parval(13)
        GRmax = parval(14)
        Gsmax = parval(15)
        ...
        JV = parval (48)
        Entrpy = parval (49)
        shcap_snow = parval (50)
        condu_snow = parval (51)
        condu_b = parval (52)
        albedo_snow = parval (53)
        fa = parval (54)
        fsub = parval (55)
        rho_snow = parval (56)
        decay_m = parval (57)
        ...
        &  gddonset,Q10,RL0,Rs0,Rr0,r_me,Q10pro,Omax,Tveg,f,bubprob,Vmaxfraction, &
        &  Q10rh,JV,Entrpy,shcap_snow,condu_snow,condu_b,albedo_snow,fa,fsub,rho_snow,decay_m,depth_ex
808     format((i7),",",36(f15.4,","),(f15.4))
    else
        r_me=parval(36)                 
        Q10pro=parval(37)   
        Omax=parval(39)    
...
!         update parameters
        do k1=1,npara
            parval(coefindex(k1))=coef(k1) ! only the parameters for DA updated their values, others remain initial values 
        enddo
!        if (do_co2_da) then
        if (do_da) then
            ...
            decay_m = parval (57)
            depth_ex = parval (58)           
!            write (*,*) 'shcap_snow',shcap_snow,'condu_snow',condu_snow,'condu_b',condu_b,'r_me',r_me,'SLA',SLA
...
!        write (*,*) 'before sub shcap_snow',shcap_snow,'condu_snow',condu_snow
        call TECO_simu(MCMC,Simu_dailyflux,Simu_soilwater,      &
     &        yrlim,dylim,Ttreat,CO2treat,              &
     &        forcing_data,yr_length,year_seq,doy_seq,hour_seq,lines,   &
     &        fwsoil,topfws,omega,wcl,Storage,nsc,yrs_eq,QC,    &
     ...
     &    do_da,do_fcast,do_snow,do_soilphy,do_co2_da,do_methane_da,do_methane_fcast,do_co2_fcast,do_EBG,output_data,dpatm,&
     &    first_year,yr_length1,nyrmax,is_grass,daily)
!        write (*,*) 'after sub shcap_snow',shcap_snow,'condu_snow',condu_snow
!         M-H algorithm
        tmp_up=upgraded
...
! ..int CWE version cost function
        call costFObsNee(Simu_dailyflux,Simu_soilwater,Simu_soiltemp,Simu_dailysoilt, &
...
!                            write (*,*) 'i',i,'Simu_dailyflux',Simu_dailyflux(2,i)
                        enddo
602                    format((i7),",",(i7),",",12(f15.4,","),(f15.4))
                        close(62)
...
!                        do i=1,daily
!                            write(70,700)i,Simu_dailyCH4(1,i),Simu_dailywatertable(1,i), &
!                            &   (Simu_dailywater(j,i),j=1,3),(Simu_dailysoilt(j,i),j=1,3)
!                        enddo
!700                 format((i7),",",7(f15.4,","),(f15.4))
!                        close(70)
...
!             rain_d,transp_d,evap_d
              ...
              do i=1,dtimes
                  if (yr .eq. 1 .and. days .ge. 113.) then                      !dbmemo15641                  
!                    write (*,*) 'days',days,'i',i
                  endif 
...
!   *** .int
!                 added for soil thermal      recording the hourly simu_soilwater only when there is obs data   Shuang
...
!            Simu_dailywater(21:30,daily)=ice(1:10)
            Simu_dailywater(21:30,daily)=wsc(1:10)
...
!            if (.not. do_co2_da) then
            if (.not. do_da) then
                ...
                elseif (do_co2_fcast) then
                        ...
                        write(*,*)'annual_summary:',year,'LAI',LAI,'annual gpp',gpp_yr,'annual Ra',Ra_yr,'annual Rh',Rh_yr, &
                        &   'leaf carbon',Pool1,'wood carbon',Pool2,'root carbon',Pool3
!                    write(*,*)'annual_summary:',yr,year,'LAI',LAI,'gpp',gpp_yr,'radsol',radsol_yr,&
...
!                        &   'P1',Pool1,'P2',Pool2,'P3',Pool3,'P4',Pool4,'P5',Pool5,'P6',Pool6,'P7',Pool7,'P8',Pool8
...
!    phi = FLDCAP  ! quick approaxy value for phi
    zmax = 300   !maximum water table depth   mm
    thetasmin = 0.25    !minimum volumetric water content at the soil surface   cm3/cm3
!    thetasmin = wiltpt  ! quick approaxy value for minimum volumetric water content at the soil surface   cm3/cm3
...
!184   format(14(f15.9,","))     
    ...
    ! Calculate increment DELTA.
      DELTA=resdh/drsdh
      sftmp_pre=sftmp
      !print*,'sftmp',sftmp,DELTA,drsdh,resdh
      sftmp=sftmp-DELTA
...
! *******2*************************       
!       write (*,*) 'in difu before _V CH4(1234)', CH4(1),CH4(2),CH4(3),CH4(4)
       do i=1,nlayers
...
!           write (77,*) 'in PMT i',i,'CH4(i)',CH4(i),'CH4_V(i)',CH4_V(i)
      enddo
!      write (*,*) 'after pmt CH4(123)', CH4(1),CH4(2),CH4(3),'Tveg',Tveg,'FRLEN_PMT(1)',FRLEN_PMT(1),'fgrow',fgrow
...
! Subroutine 1. Read parameters from file
    ...
    &   f,bubprob,Vmaxfraction, &       !..int added for methane module
    &   Q10rh,JV,Entrpy, &                   ! added for acclimation study Feb 19 2019
    &   shcap_snow,condu_snow,condu_b,albedo_snow,fa,fsub,rho_snow,decay_m,depth_ex)
    implicit none
    real lat,longi,wsmax,wsmin                  
    real LAIMAX,LAIMIN,rdepth,Rootmax,Stemmax
    real SapR,SapS,SLA,GLmax,GRmax,Gsmax,stom_n
...
!   *** .int  added for par in methane module    
    ...
    real shcap_snow,condu_snow,condu_b,albedo_snow,fa,fsub,rho_snow,decay_m,depth_ex
!    integer first_year
...
!   *** ..int added for pars in methane module
    ...
    read(10,11)commts
    read(10,*)shcap_snow,condu_snow,condu_b,albedo_snow,fa,fsub,rho_snow,decay_m,depth_ex
!    read(10,11)commts
!    read(10,*)first_year
!   ***     
    lat = 47.5
...
!   ***            
    ...
    real soiltsurf,soilt0,soilt5,soilt10,soilt30,soilt50,soilt75,soilt160,soilt200	
    ...
    character(len=150) commts,obsfile_cflux,obsfile_cpool,obsfile_sw,obsfile_st,obsfile_soilprofc,&
...
!              if(mod(m,2) .eq. 0)then
    ...
    !             obs_cflux(3,m/2)=hour
             ...
             !   ***
                 std_cflux(1,m)=-9999
                 std_cflux(2,m)=-9999
    !             std_cflux(3,m/2)=-9999

                 if(gpp .gt. -999)then
                     if (gpp_sd .ne. 0.) then
                         std_cflux(3,m)= gpp_sd ! gpp_sd  !change value here
                     else
                 ...
                 if(nee .gt. -999)then
                     if (nee_sd .ne. 0.) then
                         std_cflux(4,m)= nee_sd ! nee_sd !change value here
                     else
                 ...
                 if(reco .gt. -999)then
                     if (reco_sd .ne. 0.) then
                         std_cflux(5,m)= reco_sd !Field data, 1 value per observation !reco_sd !change value here
                     else
...
!                obs_ch4flux(1,m)=real(year)
                ...
                if(CH4_flux_sd .gt. -999)then
                    std_ch4flux(2,m)= CH4_flux_sd  !Field data, 1 value per observation (variability among all chamber systems)
                else
...
!   Observed daily soil water data	131
    if(file_exists_sw)then
	  ...
	  do
             ...
             if(sw1 .gt. -1000 .and. sw1 .lt. 1000)then
                std_soilwater(3,m)=sw1_sd !
             else
                std_soilwater(3,m)=-9999
             endif
             if(sw2 .gt. -1000 .and. sw2 .lt. 1000)then
                std_soilwater(4,m)=sw2_sd !
             else
...
!          print*,'m=',m
!              if(mod(m,2) .eq. 0)then
                 read(132,*,IOSTAT=istat1)sdoy,soilt0,soilt5,soilt10,soilt30,soilt50,soilt75,soilt160,soilt200
!              end if
              if(istat1<0)exit
              m=m+1
!              if(mod(m,2) .eq. 0)then
                 obs_soilt(1,m)=real(sdoy)
    !             obs_soilt(3,m/2)=real(hour)
                 if(soilt0 .gt. -100 .and. soilt0 .lt. 200)then
                    obs_soilt(2,m)=soilt0
                 else
                    obs_soilt(3,m)=-9999
                 end if                 
                 ...
                 if(soilt30 .gt. -100 .and. soilt30 .lt. 200)then
                    obs_soilt(5,m)=soilt30
                 else
                    obs_soilt(5,m)=-9999
                 end if
                 if(soilt50 .gt. -100 .and. soilt50 .lt. 200)then
                    obs_soilt(6,m)=soilt50
                 else
                    obs_soilt(6,m)=-9999
                 end if
                 if(soilt75 .gt. -100 .and. soilt75 .lt. 200)then
                    obs_soilt(7,m)=soilt75
                 else
                    obs_soilt(7,m)=-9999
                 end if
...
!   Observed daily snow_depth	133
    if(file_exists_snow)then
	   ...
	   std_snow(1:2,:) = -9999
!           std_snow(3,:)   = 2. !default 0811 ms
           std_snow(3,:)   = 0.337 ! STD of the time series. Units? At the moment snow is in meters!
           len_snow=m
    end if
...
!   Observed daily water table	134
    if(file_exists_wt)then
       ...
       len_wt=m
       std_wt(1:2,:)=-9999
       std_wt(3,:)=30.75160156 ! units in mm
    end if 
!   *****************************
...
! add subroutines for CWE int    
  ...
  !Subroutine  Read water table from file   Yuanyuan   
    ...
    character(len=150) watertablefile,commts
    integer m,n,istat1,lines,yr_length
...
!     skip 2 lines of input met data file
    ...
    integer, parameter :: miterms=29
    real output_data(miterms,ilines)
    character(len=150) commts,outdir,outfile
    integer m,n,istat5
...
803     format(a80) 
 ...
 !Subroutine  Read snow_depth from file   Yuanyuan   
    ...
    integer,dimension(ilines):: year_seq,doy_seq,hour_seq
    real snow_in(ilines)
    character(len=150) snowdepthfile,commts
    integer m,n,istat1,lines,yr_length
...
! Subroutine: Read Data assimilation check box file
    ...
    character(len=150) DAparfile,commts
    ...
    read(15,11)commts
    read(15,*)DApar(50),DApar(51),DApar(52),DApar(53),DApar(54),DApar(55),DApar(56),DApar(57),DApar(58)    
!    read(15,11)commts
!    read(15,*)DApar(50)
    ...
    read(15,11)commts
    read(15,*)parmin(50),parmin(51),parmin(52),parmin(53),parmin(54),parmin(55),parmin(56),parmin(57),parmin(58)
    ...
    read(15,11)commts
    read(15,*)parmax(50),parmax(51),parmax(52),parmax(53),parmax(54),parmax(55),parmax(56),parmax(57),parmax(58)
...
!    	cost function for observed data
    ...
    subroutine costFObsNee(Simu_dailyflux,Simu_soilwater,Simu_soiltemp,Simu_dailysoilt, &
		 & Simu_dailywatertable,Simu_dailywater,Simu_snowdepth,Simu_TD,Simu_dailyCH4, &
                 & obs_cflux,obs_cpool,obs_ch4flux,obs_ch4conc, &
		 & obs_soilwater,obs_soilt,obs_snow,obs_wt,obs_td,obs_soilprofc, &
                 & std_cflux,std_cpool,std_ch4flux,std_ch4conc, &
...
!			 write (*,*) 'wood dObsSim',dObsSim,'std_cpool(5,i)',std_cpool(5,i)
         ...
         !   ************ ch4 data cost function  inside co2 
...
!  !  print*,'file_exists426',file_exists,len_st
   ...
   ! end if
	...
	if(use_soilt_ob .eq. 1)then		!soilt5	soilt10	soilt20	soilt40	soilt80	soilt160	soilt200
   	    day=int(obs_soilt(1,i))
            if(obs_soilt(2,i).gt.-999.)then
               j_ds1=j_ds1+1 
               dObsSim=Simu_dailysoilt(1,day)-obs_soilt(2,i)	!Simu_dailysoilt(1), surface
               J_dsoilt1=J_dsoilt1+(dObsSim*dObsSim)/(2*std_soilt(2,i)*std_soilt(2,i))
!               write (*,*) 'dObsSim',dObsSim,'Simu_dailysoilt(1,day)',Simu_dailysoilt(1,day),'obs_soilt(2,i)',obs_soilt(2,i)
            endif
                                                                ! I did not match your -5cm soil temperature to modeled results
            if(obs_soilt(4,i).gt.-999.)then
               j_ds2=j_ds2+1 
               dObsSim=Simu_dailysoilt(2,day)-obs_soilt(4,i)		!Simu_dailysoilt(3,i),10cm
               J_dsoilt2=J_dsoilt2+(dObsSim*dObsSim)/(2*std_soilt(4,i)*std_soilt(4,i))
!               write (*,*) 'dObsSim',dObsSim,'Simu_dailysoilt(2,day)',Simu_dailysoilt(2,day),'obs_soilt(4,i)',obs_soilt(4,i)
            endif
            if(obs_soilt(5,i).gt.-999.)then
               j_ds3=j_ds3+1
               dObsSim=Simu_dailysoilt(4,day)-obs_soilt(5,i)
               J_dsoilt3=J_dsoilt3+(dObsSim*dObsSim)/(2*std_soilt(5,i)*std_soilt(5,i))      !Simu_dailysoilt(4,i),30cm
            endif
            if(obs_soilt(6,i).gt.-999.)then
               j_ds4=j_ds4+1 
               dObsSim=Simu_dailysoilt(6,day)-obs_soilt(6,i)
               J_dsoilt4=J_dsoilt4+(dObsSim*dObsSim)/(2*std_soilt(6,i)*std_soilt(6,i))!Simu_dailysoilt(5,i),50cm
!               write (*,*) 'dObsSim',dObsSim,'Simu_dailysoilt(6,day)',Simu_dailysoilt(6,day),'obs_soilt(6,i)',obs_soilt(6,i)
            endif
            if(obs_soilt(7,i).gt.-999.)then
                j_ds5=j_ds5+1 
               dObsSim=(Simu_dailysoilt(7,day)+(Simu_dailysoilt(8,day)-Simu_dailysoilt(7,day))/4)-obs_soilt(7,i)
               J_dsoilt5=J_dsoilt5+(dObsSim*dObsSim)/(2*std_soilt(7,i)*std_soilt(7,i))!Simu_dailysoilt(7&8,i),75cm
            endif								
        endif
    enddo
    !close(426)
